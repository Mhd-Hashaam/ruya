...
impl MpvCore {
  ...
  pub fn init(
    ...
    inner.backend.init(&runtime.api, runtime.handle, window_hwnd)?;
  ...
  pub fn get_property(&self, name: &str) -> Result<serde_json::Value, MpvCoreError> {
    self.with_runtime(|rt| {
      let name_c = std::ffi::CString::new(name).map_err(|_| MpvCoreError::Command("Invalid property name".into()))?;
      let res = unsafe { (rt.api.mpv_get_property_string)(rt.handle, name_c.as_ptr()) };
      if res.is_null() {
        return Ok(serde_json::Value::Null);
      }
      let val = unsafe { std::ffi::CStr::from_ptr(res) }.to_string_lossy().into_owned();
      unsafe { (rt.api.mpv_free)(res as *mut c_void); }
      ...
      if val == "yes" || val == "true" { return Ok(serde_json::Value::Bool(true)); }
      if val == "no" || val == "false" { return Ok(serde_json::Value::Bool(false)); }
  ...
  pub fn render_frame_rgba(&self, width: u32, height: u32) -> Result<Vec<u8>, MpvCoreError> {
    ...
    let w = i32::try_from(width).map_err(|_| MpvCoreError::RenderInit("width overflow".into()))?;
    let h = i32::try_from(height).map_err(|_| MpvCoreError::RenderInit("height overflow".into()))?;
    ...
    // --- Lock scope: only the actual render call holds the Mutex ---
    {
      ...
      let rc = unsafe {
        (rt.api.mpv_render_context_render)(rt.render_ctx, params.as_mut_ptr())
      };
      if rc < 0 {
  ...
  fn with_runtime<T>(
    ...
    let rt = inner
      .runtime
      .as_ref()
      .ok_or(MpvCoreError::NotInitialized)?;
    f(rt)
  }
}
...
fn create_sw_render_context(rt: &mut MpvRuntime) -> Result<(), MpvCoreError> {
  ...
  let rc = unsafe {
    (rt.api.mpv_render_context_create)(&mut ctx, rt.handle, params.as_mut_ptr())
  };
  if rc < 0 || ctx.is_null() {
...
fn run_command(rt: &MpvRuntime, args: &[&str]) -> Result<(), MpvCoreError> {
  let cstrings: Result<Vec<CString>, _> = args
    .iter()
    .map(|s| CString::new(*s))
    .collect();
