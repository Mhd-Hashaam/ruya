...
struct MpvInner {
    child: Option<Child>,
    /// Windows IPC path mpv was started with (e.g. `\\.\pipe\ruya-mpv-<id>`).
...
#[tauri::command]
pub fn mpv_process_start(app: AppHandle, controller: State<MpvController>) -> Result<(), String> {
    ensure_mpv_running(&app, &controller)
}

#[tauri::command]
pub fn mpv_process_load_file(
    app: AppHandle,
    controller: State<MpvController>,
    path: String,
) -> Result<(), String> {
...
#[tauri::command]
pub fn mpv_process_toggle_pause(
    app: AppHandle,
    controller: State<MpvController>,
) -> Result<(), String> {
    ensure_mpv_running(&app, &controller)?;
...
#[tauri::command]
pub fn mpv_process_stop(app: AppHandle, controller: State<MpvController>) -> Result<(), String> {
    ensure_mpv_running(&app, &controller)?;
    send_ipc_command(
        &controller,
        serde_json::json!({
