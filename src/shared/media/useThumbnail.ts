...
import { isBrowserNativeVideo } from "@/shared/platform/videoStreamClient";
...
function generateThumbnail(filePath: string, seekTime?: number): Promise<string> {
  return new Promise((resolve, reject) => {
    const src = convertFileSrc(filePath, "stream");
...
export function useThumbnail(filePath: string, seekTime?: number): string | null {
  ...
  useEffect(() => {
    if (!isBrowserNativeVideo(filePath)) return;
