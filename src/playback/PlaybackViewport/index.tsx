...
import {
  ...
  mpvListenFrameReady,
  mpvGetFrameBinary,
} from "@/shared/platform/mpvClient";
import {
  isBrowserNativeVideo,
  toStreamUrl,
} from "@/shared/platform/videoStreamClient";
import { usePlaybackStore } from "@/playback/playbackStore";
import { MinimalVideoControls } from "@/playback/MinimalVideoControls";
...
const M2HardwarePlayer = ({ title }: M2HardwarePlayerProps) => {
  ...
  useEffect(() => {
    const checkPip = () => {
      const pipVideo = (window as any)._pipVideo;
...
export const PlaybackViewport = ({ onBack }: PlaybackViewportProps) => {
  ...
  useEffect(() => {
    ...
    const needsMpv =
      target.kind === "audio" ||
      (target.kind === "video" && !isBrowserNativeVideo(target.path));
  ...
  const useFallbackPath = isVideo && !useNativePath;
