...
import { useVolumeBoost } from "@/shared/media/useVolumeBoost";
import { VolumeControl } from "@/shared/ui/VolumeControl";
import { usePlaybackStore } from "@/playback/playbackStore";
import { mpvGetProperty, mpvSeek, mpvStop, mpvTogglePause, mpvSetVolume, mpvSetLoop } from "@/shared/platform/mpvClient";
...
export const MinimalVideoControls = ({
  ...
  const seekBack = () => {
    if (isM2) {
      void mpvSeek(Math.max(0, currentTime - SEEK_SECONDS));
    } else {
      seekNative(Math.max(0, currentTime - SEEK_SECONDS));
    }
  };

  const seekFwd = () => {
    if (isM2) {
      void mpvSeek(Math.min(duration, currentTime + SEEK_SECONDS));
    } else {
      seekNative(Math.min(duration, currentTime + SEEK_SECONDS));
    }
  };
  ...
  const togglePiP = async () => {
    if (isM2) {
      ...
      let pipVideo = (window as any)._pipVideo as HTMLVideoElement | undefined;
      if (!pipVideo) {
        pipVideo = document.createElement("video");
        pipVideo.muted = true; // Audio is handled natively by mpv
        pipVideo.autoplay = true;
        (window as any)._pipVideo = pipVideo;
      }
